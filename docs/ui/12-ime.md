...
| **換行** | 不允許 | OK（idle 時插入 `\n`）|
| **字數計數位置** | 框內右側 | 編輯區右下角 |
| **OK** | 確認 → 回 App | 候選字 commit / multitap commit / idle 換行（**非送出**）|
| **送出** | n/a | **SET 短按** = `view_router_modal_finish(true)` |
...
| **OK** | 確認 → 回 App | 候選字 commit / multitap commit / idle 插 `\n` 換行 |
| **SET** | （不在 IME 內，無作用） | **送出 / 套用** → 回 App[^1] |
...
[^1]: 詳見 §模式 B 送出/換行（v2.0 已收斂）。OK 長按送出方案已否決，SET 兼任送出。
...
| 鍵 | 行為 |
|---|---|
| **SET 短按** | **送出 / 套用**（`view_router.c:415-424` `view_router_modal_finish(true)`，回呼 `committed=true`）|
| **BACK** | 取消；若該編輯器有 `draft_id`，自動存草稿（`modal_trampoline`，`ime_task.cpp:553-555`）|
| **OK** | 候選字 → commit selected；multitap pending → commit；idle → 插入 `\n` 換行（`ime_logic.cpp:152` Phase 1.4 Task C）|

**選擇 SET 而非 OK 短/長按的理由**：

最初規劃「OK 短=換行 / OK 長 ≥500 ms=送出」與其後考慮過的二段確認、專屬送出鍵、合併編輯模式都被否決。SET 兼任送出方案勝出的關鍵：

1. **語意風險可控**：SET 在 Op 態原本是「進當前 App 子設定」。在模式 B 內 SET 顯然不會去進子設定（已在 IME 內），語意衝突只是文字上重疊，使用者不會混淆。
2. **無時間判讀負擔**：短按即送出，不需倒數條或長按等待。
3. **誤觸成本低**：SET 與 OK 在實體鍵盤上分屬左右兩側功能鍵群（SET 在右上、OK 在 D-pad 中央），手指位置不會誤觸。
4. **保留 OK 換行能力**：對話訊息常用換行，把 OK 留給高頻動作；送出走相對低頻的 SET 更符合使用情境。
5. **BACK 存草稿、SET 送出** 形成對稱：兩個動作都是「離開編輯器」，但結果不同（discard-with-save vs publish）。

**否決方案的歷史紀錄**（保留以避免重蹈）：

| 方向 | 否決理由 |
|---|---|
| OK 長按 ≥500 ms 送出 | 無視覺回饋；誤觸代價極高（不完整訊息廣播全 mesh 不可收回）；與 BACK 長按鎖屏的低代價不對稱 |
| 專屬送出鍵（R4 某位） | 鍵位已滿；需重新規劃功能鍵布局，影響面太大 |
| 合併編輯模式（取消 A/B 分界） | 對話框中失去隨手換行能力；短訊息常用 |
| 二段確認浮層 | 多一步操作；雖符合「不可逆動作需確認」但拖慢高頻送出 |

✅ **狀態**：v2.0 已實作落地（messages_send / settings_view 文字編輯 / channel_add name 編輯 等所有模式 B 觸發點均走此約定）。
...
最後更新：2026-04-30
版本：v2.0（模式 B 送出鍵收斂為 SET；BACK 存草稿、OK 換行；候選字翻頁/multitap 行為對齊 ime_logic.cpp 實作）

v1.1（2026-04-29）：OK 長按邏輯撤回；對齊 MIE 實際 API：callback+polling、三模式 SmartZh/SmartEn/Direct、無 Num、multitap 800ms、kPageSize 待自適應化、無 CapsLock；移除 MQTT/Wi-Fi 文字框；草稿位址規劃；候選字數依寬度自適應
