...
| P2-16 | 2026-04-22 | Flash stuck at 25 MHz / CLKDIV=3 — bootrom default PAD SLEWFAST=0 throttles SCK edge at ≥75 MHz | **Symptom:** `flash_bench` at bootrom-default M0 (CLKDIV=3 / RXDELAY=2) gives 20.5 MB/s uncached, 20.5 MB/s cached (cache barely helps at 25 MHz because cmd+addr overhead dominates). `flash_sweep2` at CLKDIV=1 (75 MHz) FAILs across all 8 RXDELAY values with deterministic nibble-shifted reads (0x01808000, 0x1D000808, 0xD1018080 …) — values change with RXDELAY + DUMMY_LEN + SUFFIX M-byte but never hit expected 0x20082000. Contradicts the observation that **PSRAM on the same QMI bus (shared SCK/SD[3:0]) works perfectly at 75 MHz** (108 M-word 6-pattern stress, 0 errors — see P2-15). **Investigation path:** (1) 36-combo `flash_sweep2` sweep at 75 MHz (DUMMY 16/20/24/28 × SUFFIX none/0xA0/0xF0 × RXDELAY 1/2/3): all FAIL with consistent nibble-shift signature → rules out register-level timing misconfiguration. (2) Confirmed W25Q128JW non-DTR variant does NOT support full QPI (4-4-4 mode) — datasheet §9 explicitly directs to separate DTR datasheet; only SPI / 1-1-4 / 1-4-4 available. Sending `0x35` expecting "Enter QPI" was a misstep — on W25Q that's `Read Status Register-2`, not Enter QPI (APS6404L conventions ≠ W25Q conventions). (3) Tested `Fast Read Quad Output (6Bh)` 1-1-4 mode at 75 MHz RXDELAY 1-5 — still all FAIL, same shift signature. (4) Hypothesised chip-side drive strength: wrote W25Q128JW SR3 via volatile path (0x50 + 0x11) to change DRV1/DRV0 from 0b11 (25% default) to 0b00 (100%). Verified via SR3 readback. **Zero effect** on 75 MHz reads — same bit patterns. Ruled out flash-side output drive. (5) Then checked `PADS_QSPI` registers via SDK header: `GPIO_QSPI_SCLK` defaults = DRIVE=1 (4 mA), SLEWFAST=0 (slow), SCHMITT=1 (on); `GPIO_QSPI_SD0..SD3` default SCHMITT=1. **Pico SDK `boot2_w25q080.S` sets SCLK DRIVE=2 (8 mA), SLEWFAST=1, clears SD SCHMITT** — but boot2 doesn't run on RP2350 by default (see P2-13), so our pads stay at bootrom defaults. PSRAM in QPI mode drives all 4 lines simultaneously during cmd+addr (no single→quad transition); 1-4-4 flash drives only SD0 during cmd then expands to SD0-3 — the bit-width transition stresses SCK edge alignment at 75 MHz. **Ablation (definitive):** 2³ combos at CLKDIV=1 / RXDELAY=2: SLEWFAST=0 → 4/4 FAIL regardless of DRIVE or SCHMITT; SLEWFAST=1 → 4/4 PASS at identical ~42.8 MB/s uncached. Throughput is flat across all PASS combos (within 0.3% noise). **Root cause:** single bit — `PADS_QSPI.GPIO_QSPI_SCLK.SLEWFAST` must be 1 for 75 MHz 1-4-4 flash operation. Slow-slew SCK eats ~3-5 ns of the 13.3 ns period, leaving flash internal setup/hold without margin; fast-slew cuts edge to ~1-2 ns and restores margin. | **Fix plan (2026-04-22):** single-line patch in `variant.cpp:initVariant()`, colocated with P2-13 XIP cache enable: `*(volatile uint32_t *)(PADS_QSPI_BASE + PADS_QSPI_GPIO_QSPI_SCLK_OFFSET + REG_ALIAS_SET_BITS) = PADS_QSPI_GPIO_QSPI_SCLK_SLEWFAST_BITS;`. Plus runtime M0 reconfig: `qmi_hw->m[0].timing = 0x20000201` (CLKDIV=1, RXDELAY=2, COOLDOWN=1), rfmt/rcmd unchanged. Expected production gain: +117% cached flash read (20.5 MB/s → 44 MB/s), significantly faster Core 0 Meshtastic instruction fetch. Keep DRIVE at 4 mA and SD SCHMITT on (ablation showed unnecessary; ship with minimum-footprint change). **Status:** validated on bringup (`flash_pad_ablation` clean PASS across 4 SLEWFAST=1 combos); production port pending. |
