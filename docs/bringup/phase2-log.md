...
| P2-16 | 2026-04-22 | Flash stuck at bootrom default 25 MHz — CLKDIV=1 not viable on W25Q128JW (1.8 V) + this Rev A PCB | **Symptom:** `flash_bench` at bootrom-default M0 (CLKDIV=3 / RXDELAY=2) gives 20.5 MB/s uncached, 20.5 MB/s cached (cache barely helps at 25 MHz because cmd+addr overhead dominates). `flash_sweep2` at CLKDIV=1 (75 MHz) FAILs across all 8 RXDELAY values with deterministic nibble-shifted reads (0x01808000, 0x1D000808, 0xD1018080 …). Contradicts observation that PSRAM on the same QMI bus (shared SCK/SD[3:0]) works fine at 75 MHz (see P2-15). **Investigation path:** (1) 36-combo `flash_sweep2` sweep at 75 MHz (DUMMY 16/20/24/28 × SUFFIX none/0xA0/0xF0 × RXDELAY 1/2/3): all FAIL — rules out register-level timing alone. (2) Confirmed W25Q128JW non-DTR variant does NOT support full QPI (4-4-4 mode) — datasheet §9 explicitly punts to DTR datasheet. Sending `0x35` expecting "Enter QPI" is wrong — on W25Q it's "Read Status Register-2", not Enter QPI (APS6404L conventions ≠ W25Q). (3) Tested 1-1-4 mode (6Bh) at 75 MHz — same shift signature. (4) Flash-side SR3 DRV 25 %→100 %: zero effect. (5) Discovered RP2350 QSPI pad defaults (DRIVE=4 mA, SLEWFAST=0, SCHMITT=1) are NOT optimised for 75 MHz because boot2 doesn't run on RP2350. First ablation on 2³ (DRIVE × SLEWFAST × SCHMITT) with a one-word sentinel check reported "SLEWFAST=1 alone is enough, 42.8 MB/s". Shipping that to production HardFaulted inside FreeRTOS `vStartFirstTask` at 0x1002B8A6 — SWD trace showed flash returning `0x0000BD10` (`pop {r4,pc}`) where the ELF has `svc 0x66`. (6) Wrote `flash_deep_scan` to compare full 16 MB block-XOR at CLKDIV=3 vs CLKDIV=1 + candidate pad config. At SLEWFAST=1 alone, 120/256 (47 %) blocks mismatch — the one-word ablation was a false positive. Added `flash_deep_ablation` across 4 pad combos: **SLEWFAST alone = 120 FAIL; SLEWFAST + DRIVE=8 mA = 0 FAIL (whether SCHMITT on or off).** (7) Production with DRIVE=8 mA + SLEWFAST=1 + CLKDIV=1 + RXDELAY=2 **still HardFaults** inside vStartFirstTask, despite bringup deep_scan reporting 0 errors under the same register state. Stack frame analysis shows a precise bus fault on the SVC 0x66 instruction (stacked PC at the following nop), meaning the CPU faulted either during SVC vector fetch or handler entry. (8) Flash reads at 0x1002B8A0 now show correct data under SWD (post-8 mA), yet boot fault is identical. Symptom is consistent with a probabilistic/workload-dependent failure that the single-pass deep_scan oracle can't detect (both baseline and CLKDIV=1 reads may share an identical-but-wrong cache fill on any single pass). **Device context:** W25Q128JW is the 1.8 V variant; forum reports on other 1.8 V custom RP2040 boards required CLKDIV=/4 for stability. Pico 2 stock uses 3.3 V W25Q32RV, Pimoroni Pico Plus 2 uses 3.3 V W25Q128JV — both run CLKDIV=2 / 75 MHz reliably. On this Rev A PCB the 1.8 V + trace length + pad drive combination sits at the margin. | **Shipped (2026-04-22): NO runtime change.** Flash stays at bootrom CLKDIV=3 / 25 MHz with default pads. The `flash_retime_clkdiv1()` helper is kept in `variant.cpp` as unused code with a long inline note so the investigation state is obvious to the next maintainer. **Bringup tooling landed:** `flash_bench`, `flash_sweep2`, `flash_pad_ablation`, `flash_deep_scan`, `flash_deep_ablation`, `flash_reset`, `flash_boost_pads`, `qmi_diag`. **Rev B recommendations:** (a) use 3.3 V W25Q128JV variant if pin-compatible or (b) shorten QSPI traces + improve return-path grounding. Either should unlock the +117 % cached flash read gain (20.5 → 44 MB/s) the bringup ablation promised. |
