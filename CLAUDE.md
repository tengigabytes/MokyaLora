...
### DM testing — PKI sync after every flash

Each `bash scripts/build_and_flash.sh` does **not** rotate the MokyaLora's
own pubkey (lives in deviceState region, survives flashes). But peer
nodes' caches of MokyaLora's pubkey, and MokyaLora's cache of peer
pubkeys, drift over time / across reboots / across fresh build_and_flash
cycles. Before any DM (`--sendtext --dest '!xxxx'`) test, run the
explicit NodeInfo-with-want_response exchange from `scripts/nodeinfo_request.py`
(both directions) and verify each side's `--info` shows a non-empty
`publicKey` for the other node. Without this, DM returns
`PKI_SEND_FAIL_PUBLIC_KEY` or `PKI_UNKNOWN_PUBKEY` even when the LoRa
RF path is fine. Full procedure + diagnostic table in
`docs/bringup/phase2-log.md` "PKI sync procedure" subsection (under
M5 closure). Note: Meshtastic's periodic NodeInfo broadcast is the
"lite" form (no pubkey); only the want_response-triggered reply carries
the pubkey, so waiting won't help — must run the helper.
