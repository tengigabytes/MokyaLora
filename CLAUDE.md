...
- Current status: **M1.1-B ✅ complete — Milestone 1 closed.** `meshtastic --port COM16 --info` completes a full protobuf round-trip through Core 1's USB CDC bridge, returning Owner / Metadata / the full 86-node mesh list. Every byte crosses `firmware/shared/ipc/` SPSC rings — Core 0 Meshtastic talks to `IpcSerialStream` which pushes into `c0_to_c1`, Core 1 `m1_bridge` (Pico SDK + FreeRTOS RP2350_ARM_NTZ + TinyUSB CDC, loaded from flash `0x10200000` via `multicore_launch_core1_raw()`) drains the ring into CDC IN and pushes CDC OUT back through `c1_to_c0`. Custom `core1_reset.S` bypasses the SDK `crt0.S` CPUID check, and SWD breadcrumbs live at `0x2007FFC0..0x2007FFD0` inside `g_ipc_shared._tail_pad` (moved from the legacy `0x20078000` window which turned out to be inside Core 0's FreeRTOS heap — see Issue P2-4). M2 (interrupt-driven IPC + Core 1 LVGL UI) is next.
