...
void convolve(uint32_t Signal[], unsigned short SignalLen,
              uint32_t Kernel[], unsigned short KernelLen,
              uint32_t Result[])
{
  uint16_t n;
...
void Open_PDM_Filter_Init(TPDMFilter_InitStruct *Param)
{
  ...
  sub_const = sum >> 1;
  div_const = (uint32_t)(sub_const * Param->MaxVolume / 32768 / FILTER_GAIN);
...
void Open_PDM_Filter_64(uint8_t *data, uint16_t *dataOut, uint16_t volume, TPDMFilter_InitStruct *Param)
{
  ...
  for (i = 0, data_out_index = 0; i < Param->Fs / 1000; i++, data_out_index += channels) {
    ...
    dataOut[data_out_index] = (uint16_t)Z;
    data += data_inc;
  }
