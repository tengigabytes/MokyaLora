...
// ---------------------------------------------------------------------------
// TFT PIO helpers
// (Local static copies; bringup_tft.c counterparts are static and not shared.)
// ---------------------------------------------------------------------------

static uint g_sm, g_off;

static bool g_tft_start(void) {
    int n = pio_claim_unused_sm(pio1, false);
    if (n < 0) { printf("ERROR: no free PIO1 SM\n"); return false; }
    g_sm = (uint)n;
    if (!pio_can_add_program(pio1, &tft_8080_program)) {
        printf("ERROR: PIO1 program full\n");
        pio_sm_unclaim(pio1, g_sm);
        return false;
    }
    g_off = pio_add_program(pio1, &tft_8080_program);
    tft_8080_program_init(pio1, g_sm, g_off, TFT_D0_PIN, TFT_nWR_PIN, 4.0f);
    pio_sm_set_enabled(pio1, g_sm, true);
    return true;
}

static void g_tft_stop(void) {
    pio_sm_set_enabled(pio1, g_sm, false);
    pio_remove_program(pio1, &tft_8080_program, g_off);
    pio_sm_unclaim(pio1, g_sm);
    for (uint i = 0; i < 8; i++)
...
static inline void g_flush(void) {
    while (!pio_sm_is_tx_fifo_empty(pio1, g_sm)) tight_loop_contents();
    sleep_us(1);
}

static inline void g_w8(uint8_t b) { pio_sm_put_blocking(pio1, g_sm, (uint32_t)b); }

static void g_cmd(uint8_t c) {
    g_flush();
...
static inline void g_dat(uint8_t d) { g_w8(d); }

static void g_win(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    g_cmd(0x2A); g_dat(x0 >> 8); g_dat(x0 & 0xFF); g_dat(x1 >> 8); g_dat(x1 & 0xFF);
    g_cmd(0x2B); g_dat(y0 >> 8); g_dat(y0 & 0xFF); g_dat(y1 >> 8); g_dat(y1 & 0xFF);
}

static void g_rect(int x, int y, int w, int h, uint16_t c) {
    if (w <= 0 || h <= 0) return;
    g_win((uint16_t)x, (uint16_t)y, (uint16_t)(x+w-1), (uint16_t)(y+h-1));
    g_cmd(0x2C);
    uint8_t hi = c >> 8, lo = c & 0xFF;
    for (int i = 0; i < w * h; i++) { g_w8(hi); g_w8(lo); }
    g_flush();
}

// ST7789VI init (same sequence as bringup_tft.c)
static void g_tft_init(void) {
    g_cmd(0x01); sleep_ms(150);  // SWRESET
    g_cmd(0x11); sleep_ms(120);  // SLPOUT
    g_cmd(0x3A); g_dat(0x55);    // COLMOD: RGB565
    g_cmd(0x36); g_dat(0x00);    // MADCTL: portrait, RGB
    g_cmd(0xB2); g_dat(0x0C); g_dat(0x0C); g_dat(0x00); g_dat(0x33); g_dat(0x33);
    g_cmd(0xB7); g_dat(0x35);
    g_cmd(0xBB); g_dat(0x19);
    g_cmd(0xC0); g_dat(0x2C);
    g_cmd(0xC2); g_dat(0x01);
    g_cmd(0xC3); g_dat(0x12);
    g_cmd(0xC4); g_dat(0x20);
    g_cmd(0xC6); g_dat(0x0F);
    g_cmd(0xD0); g_dat(0xA4); g_dat(0xA1);
    g_cmd(0xE0);
    g_dat(0xD0); g_dat(0x04); g_dat(0x0D); g_dat(0x11); g_dat(0x13); g_dat(0x2B);
    g_dat(0x3F); g_dat(0x54); g_dat(0x4C); g_dat(0x18); g_dat(0x0D); g_dat(0x0B);
    g_dat(0x1F); g_dat(0x23);
    g_cmd(0xE1);
    g_dat(0xD0); g_dat(0x04); g_dat(0x0C); g_dat(0x11); g_dat(0x13); g_dat(0x2C);
    g_dat(0x3F); g_dat(0x44); g_dat(0x51); g_dat(0x2F); g_dat(0x1F); g_dat(0x1F);
    g_dat(0x20); g_dat(0x23);
    g_cmd(0x2A); g_dat(0x00); g_dat(0x00); g_dat(0x00); g_dat(0xEF);  // CASET 0..239
    g_cmd(0x2B); g_dat(0x00); g_dat(0x00); g_dat(0x01); g_dat(0x3F);  // RASET 0..319
    g_cmd(0x21);               // INVON (required for IPS colour correctness)
    g_cmd(0x29); sleep_ms(20); // DISPON
}

// ---------------------------------------------------------------------------
// 5x8 bitmap font — column-major, LSB = top row, chars 0x20-0x7E (95 glyphs)
// Each entry: 5 bytes, one per column; bit 0 = topmost pixel of that column.
// Based on the classic Arduino/Adafruit 5x7 font, public domain.
// ---------------------------------------------------------------------------

static const uint8_t FONT5[95][5] = {
...
// scale=1: 6x8 px cell (5-col glyph + 1-col gap)
// scale=2: 12x16 px cell
static void g_char(int x, int y, char c, uint16_t fg, uint16_t bg, int sc) {
    if (c < 0x20 || c > 0x7E) c = ' ';
    const uint8_t *g = FONT5[(uint8_t)c - 0x20];
    int cw = 6 * sc, ch = 8 * sc;
    g_win((uint16_t)x, (uint16_t)y,
          (uint16_t)(x + cw - 1), (uint16_t)(y + ch - 1));
    g_cmd(0x2C);
    // Pixels in RAMWR order: row-major, top-left first.
...
// Draw a string padded / truncated to exactly `width` characters.
// Clips at the right edge of the screen.
static void g_str(int x, int y, const char *s, int width,
                  uint16_t fg, uint16_t bg, int sc) {
    int cx = x, slen = (int)strlen(s);
    for (int i = 0; i < width; i++) {
        if (cx + 6 * sc > 240) break;
        char c = (i < slen) ? s[i] : ' ';
        g_char(cx, y, c, fg, bg, sc);
        cx += 6 * sc;
    }
}
...
static void back_key_init(void) {
...
static void back_key_deinit(void) {
...
static bool back_key_pressed(void) {
