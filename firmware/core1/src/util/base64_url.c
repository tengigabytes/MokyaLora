...
static const char s_alphabet_std[64] =
...
{
    if (out == NULL || cap == 0u) return 0u;
    size_t need = ((in_len + 2u) / 3u) * 4u + 1u;   /* always padded */
    if (cap < need) return 0u;
    ...
    while (i + 3u <= in_len) {
        uint32_t v = ((uint32_t)in[i] << 16) |
                     ((uint32_t)in[i + 1u] << 8) |
                      (uint32_t)in[i + 2u];
        out[o + 0] = s_alphabet_std[(v >> 18) & 0x3Fu];
        out[o + 1] = s_alphabet_std[(v >> 12) & 0x3Fu];
        out[o + 2] = s_alphabet_std[(v >>  6) & 0x3Fu];
        out[o + 3] = s_alphabet_std[(v >>  0) & 0x3Fu];
        i += 3u;
        o += 4u;
    }
    if (i + 1u == in_len) {
