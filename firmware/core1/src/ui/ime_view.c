...
typedef struct {
    ...
    int32_t  selected;
    char     words[8][24];
    /* Hex dump of candidates_[0..1] raw 16-byte prefix — captures the
     * mis-parse signature bytes (e.g. "06 E8 86 00" vs the expected
     * "E8 86 BD xx"). */
    uint8_t  raw0[16];
    uint8_t  raw1[16];
} ime_view_debug_t;
#define IME_VIEW_DEBUG_MAGIC  0xEEED0002u
...
void ime_view_apply(const key_event_t *ev)
{
    ...
    int target = -1;
    if (ev->keycode == MOKYA_KEY_UP)   target = find_row_neighbour(-1);
    if (ev->keycode == MOKYA_KEY_DOWN) target = find_row_neighbour(+1);
    if (target >= 0) {
        /* The engine already ran its own Up/Down handler (priority +3
         * vs LVGL +2) and moved `selected_` by kPageSize. Our override
         * wins because ime_view_set_selected takes the mutex and sets
         * selected to whatever row we picked. */
...
static void render_candidates(void)
{
    ...
    lv_obj_clean(s_cand_box);
    s_cell_valid = 0;
    lv_obj_t *cells[CAND_MAX] = {0};
    lv_obj_t *selected_cell = NULL;
    ...
    for (int i = 0; i < s_cand_count; ++i) {
        if (cells[i]) {
            s_cell_x[i] = (int16_t)lv_obj_get_x(cells[i]);
            s_cell_y[i] = (int16_t)lv_obj_get_y(cells[i]);
        } else {
            /* Skipped cell — park at an impossible y so it never wins
             * the "closest row" pick. */
            s_cell_x[i] = 0;
    ...
    if (selected_cell) {
        lv_obj_scroll_to_view(selected_cell, LV_ANIM_OFF);
    }
}
